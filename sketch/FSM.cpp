...
namespace HFTE
{
	 ...
	 */
	void InitFSMmatrices(std::set<TradePolicy> policies)
	{
		if (policies.size() == 0) LOGD("InitFSMMatrices() No trading policies");
		for (auto policy : policies) {
			LOGD("InitFSMMatrices() Adding trading policy {}", GetTradePolicyString(policy));
